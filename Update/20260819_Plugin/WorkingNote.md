...
9. ~~**`doubao-search` MCP key is a placeholder** (`<AGENT_PLAN_API_KEY>`)~~ — **resolved 2026-08-20**: the entry now carries the real Ark exclusive-plan key (works on the Custom版 `web_search` endpoint, verified live). Separately, `uvx` could not reach pypi.org (proxy TLS block) — fixed with a TUNA mirror in `~/.config/uv/uv.toml`; and the Global版 endpoint (`global_search`) rejects the Ark key (`700901`) and needs a dedicated 豆包搜索控制台 API key — see `Config/ReadMe.md` "Web Search MCP".

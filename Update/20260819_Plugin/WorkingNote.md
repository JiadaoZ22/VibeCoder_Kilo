...
9. **Doubao Search** will be configured manually in `~/.config/kilo/opencode.json` per the [Ark console docs](https://console.volcengine.com/ark/region:cn-beijing/docs/82379/2301412?lang=zh) — no third-party MCP server is required. A working internet/proxy setup remains necessary: this machine uses TUNA PyPI (`~/.config/uv/uv.toml`) and npmmirror (`~/.npmrc`) because pypi.org and registry.npmjs.org are TLS-blocked.
